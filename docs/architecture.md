...
`/invite/:token`. Старый `/shared` редиректит на `/wardrobes`. `/invite/:token`
обрабатывается отдельной веткой `AppShell` (своя логика auth/loading). SPA-роутинг на
проде обеспечивает `vercel.json` (рерайт всех не-`/api/` путей на `/`).
