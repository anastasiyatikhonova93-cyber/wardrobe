...
`/detect-test` (мульти-распознавание, см. [photo-pipeline.md](photo-pipeline.md)) рендерится
**всегда standalone** (единой позицией в дереве, вне `Layout`) **до auth-гейта и экрана
ошибки** — детекция не зависит от данных гардероба, поэтому доступна на preview-домене без
Firebase authorized domains и при недоступной базе. Точка входа — кнопка-иконка `ScanSearch`
в шапке `/wardrobe`; на самой странице для залогиненного есть ссылка «В гардероб» (нижней
навигации там нет). Сохранение идёт в активный гардероб из стора.
