...
- **`OutfitItemPosition`** — позиция вещи на холсте образа: `x`/`y` (% холста),
  `userScale` (ручной множитель размера, по умолчанию 1). Размер вещи = высота по
  категории (`CATEGORY_HEIGHT`) × `userScale` × реальное соотношение сторон фото.
...
образа (`CATEGORY_HEIGHT` — целевая высота вещи по категории и пр.) — в `src/lib/outfit-layout.ts`.
...
категориальный размер задаёт `CATEGORY_HEIGHT`, поэтому `userScale` у старых образов
по умолчанию `1` — это семантически верно, а не потеря данных. Сами `x/y` остаются
как есть; раскладка пересчитывается под новую геометрию (высота-по-категории ×
