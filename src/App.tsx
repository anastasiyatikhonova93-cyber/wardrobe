...
function AppShell() {
  ...
  // Мульти-распознавание — всегда standalone, единой позицией в дереве (без
  // ремоунта при смене состояния): и публичная песочница для незалогиненного
  // (работает на preview-домене без Firebase authorized domains), и точка входа
  // для залогиненного (кнопка ScanSearch в шапке /wardrobe). Рендерится ДО
  // auth-гейта и экрана ошибки — детекция (/api/detect) не зависит от данных
  // гардероба, поэтому доступна даже при недоступной базе (тот же кейс, ради
  // которого сделан серверный прокси). Возврат в приложение — ссылка на самой
  // странице (видна залогиненному); сохранение идёт в активный гардероб из стора.
